  ...
  return (
    <div className={`mb-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        {/* Left label */}
        <span className="text-xs text-[#6B4B41] font-nunito w-16 text-left">
          {getLeftLabel()}
        </span>
          ...
          {[1, 2, 3, 4].map((rating) => (
            <label key={rating} className="flex flex-col items-center cursor-pointer group">
              <input
                ...
                className="w-4 h-4 text-[#AF7C71] border-[#D7C5AA] focus:ring-[#AF7C71] focus:ring-2"
              />
        ...
        {/* Right label */}
        <span className="text-xs text-[#6B4B41] font-nunito w-16 text-right">
          {getRightLabel()}
        </span>
      </div>
